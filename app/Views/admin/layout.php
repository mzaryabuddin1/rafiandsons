...
<head>
    ...
    <title><?= esc($pageTitle ?? 'Admin') ?> | Rafi &amp; Sons</title>
    ...
    <link href="<?= base_url('admintheme/css/plugins/dataTables/datatables.min.css') ?>" rel="stylesheet">
    <link href="<?= base_url('admintheme/css/plugins/sweetalert/sweetalert.css') ?>" rel="stylesheet">
...
<script>
    $(document).on('click', '.admin-logout-btn', function (e) {
        e.preventDefault();
        AdminApp.request(ADMIN_BASE + '/logout', 'POST').done(function (res) {
            window.location.href = (res.data && res.data.redirect) || (ADMIN_BASE + '/login');
        });
    });
</script>
