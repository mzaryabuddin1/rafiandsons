...
<div class="col-lg-4 text-right"><?php if (!empty($canCreate)): ?><a href="<?= site_url('admin/roles/create') ?>" class="btn btn-primary"><i class="fa fa-plus"></i> Add Role</a><?php endif; ?></div>
</div>
...
var table = AdminApp.createDataTable({
    ...
    renderRow: function (r) {
        var a = '';
        if (canUpdate && r.is_super != 1) a += '<a class="btn btn-xs btn-primary" href="' + ADMIN_BASE + '/roles/' + r.id + '/edit"><i class="fa fa-pencil"></i></a> ';
        if (canDelete && r.is_super != 1) a += '<button class="btn btn-xs btn-danger btn-delete" data-id="' + r.id + '"><i class="fa fa-trash"></i></button>';
        if (r.is_super == 1) a += '<span class="text-muted">Locked</span>';
...
$(document).on('click','.btn-delete',function(){var id=$(this).data('id');AdminApp.confirmDelete(function(){AdminApp.request(ADMIN_BASE+'/api/roles/'+id+'/delete','POST').done(function(res){AdminApp.toast('success',res.message);table.load(true);});});});
table.load(true);
</script>
<?= $this->endSection() ?>
